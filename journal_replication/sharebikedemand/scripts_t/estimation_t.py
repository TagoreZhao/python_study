...
def gen_loc(loc_bound, grid_size, s=1, coor=np.zeros(2)):
    ...
    if isinstance(loc_bound, list):
        x_coords = np.linspace(loc_bound[0]/s + coor[0], loc_bound[1]/s + coor[0], grid_size)
        y_coords = np.linspace(loc_bound[2]/s + coor[1], loc_bound[3]/s + coor[1], grid_size)
    else:
        x_coords = np.linspace(-loc_bound/s + coor[0], loc_bound/s + coor[0], grid_size)
        y_coords = np.linspace(-loc_bound/s + coor[1], loc_bound/s + coor[1], grid_size)

    # Create a meshgrid for the x and y coordinates
    xx, yy = np.meshgrid(x_coords, y_coords)

    # Stack and reshape to get the final location array
    loc = np.stack([xx.ravel(), yy.ravel()], axis=-1).reshape(1, -1, 2)
...
def caldist(loc, bike_loc, bike_num):
    '''
    Input:
        loc: The location of the rider
        bike: The location of th bikes
        bike_num: number of bike
    Output: 
        An array that contains the Euclidean distance between arrival locations and bikes at each time period. If bike is unavailable,
        the distance is infinity.

    loc[:, np.newaxis, np.newaxis, :] expands loc into a shape suitable for broadcasting with bike_loc.
    bike_loc[np.newaxis, :, :, :] similarly expands bike_loc for broadcasting.
    '''
    # Broadcasting to calculate the Euclidean distance
    dist = np.linalg.norm(loc[:, np.newaxis, np.newaxis, :] - bike_loc[np.newaxis, :, :, :], axis=-1)
    return dist
...
def findw_EM(cur_locnum,loc,beta0,bike_num,num_records,bike_loc,book_bike,num_booked,
  ...
  while (w_diff > thres):
    ...
    #When the operator have prior knowledge regarding w, we incorporate the prior by using a maximum a posteriori probability instead of MLE.
